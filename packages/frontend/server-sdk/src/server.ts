...
export interface ForumlineServerConfig {
  ...
  getUnreadCounts?: (
    userId: string,
  ) => Promise<{ notifications: number; chat_mentions: number; dms: number }>;
  ...
  authCallbackHandler(): RequestHandler {
    return async (req, res) => {
      ...
      if (forumline_access_token) {
        setCookies.push(
          `forumline_access_token=${forumline_access_token}; Path=/; HttpOnly; SameSite=None; Secure; Max-Age=3600`,
        );
      }
