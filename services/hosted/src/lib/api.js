...
export const api = {
  ...
  isBookmarked: threadId =>
    get(`/api/bookmarks/${encodeURIComponent(threadId)}/status`, true).then(r => r.bookmarked),
  ...
  searchThreads: q =>
    q.trim() ? get(`/api/search/threads?q=${encodeURIComponent(q)}`) : Promise.resolve([]),
  searchPosts: q =>
    q.trim() ? get(`/api/search/posts?q=${encodeURIComponent(q)}`) : Promise.resolve([]),
  ...
  sendChatMessage: input =>
    post(`/api/channels/_by-id/${encodeURIComponent(input.channel_id)}/messages`, {
  ...
  updateNotificationPreference: (category, enabled) =>
    put('/api/notification-preferences', { category, enabled }),
};
