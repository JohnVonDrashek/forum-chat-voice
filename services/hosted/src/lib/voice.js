...
export async function joinRoom(slug, name) {
  ...
  try {
    ...
    if (!accessToken) {
      voiceStore.set({ connectError: 'Not authenticated', isConnecting: false });
      return;
    }
    ...
    voiceStore.set({
      ...
      isMuted: false,
      isDeafened: false,
      connectedRoomSlug: slug,
      connectedRoomName: name,
    });
  } catch (err) {
    voiceStore.set({
      connectError: err instanceof Error ? err.message : 'Failed to connect',
      isConnecting: false,
    });
  }
}
...
async function connectLiveKit(slug, displayName, accessToken) {
  ...
  room.on(lk.RoomEvent.LocalTrackPublished, pub => {
    if (
      pub.track?.source === lk.Track.Source.ScreenShare &&
      pub.track.kind === lk.Track.Kind.Video
    ) {
      voiceStore.set({
...
function updateLiveKitParticipants() {
  ...
  const remotes = Array.from(room.remoteParticipants.values()).map(p => {
    const audioTrack = p
      ...
      .find(t => t.track?.source === lk.Track.Source.Microphone);
    return {
      id: p.identity,
