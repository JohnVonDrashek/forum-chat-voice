...
import { authStore, initAuth, tokenExchange } from './lib/auth.js';
...
import { navigate, resolve, route, setNotFound } from './router.js';
...
function requireAuth(renderFn) {
  return params => {
    ...
    if (!user) {
      ...
      if (container)
        container.innerHTML =
          '<p class="text-center py-8 text-slate-400"><a href="/login" class="text-indigo-400 hover:text-indigo-300">Sign in</a> to access this page.</p>';
      return;
    }
