...
export function renderBookmarks(container) {
  ...
  if (!user) {
    container.innerHTML =
      '<p class="text-center py-8 text-slate-400"><a href="/login" class="text-indigo-400">Sign in</a> to view bookmarks.</p>';
    return;
  }
    ...
    container.innerHTML = `
      ...
      <div class="space-y-2">
        ...
        `,
                ...
                .join('')
            : '<p class="text-slate-400 text-center py-8">No bookmarks yet. <a href="/" class="text-indigo-400 hover:text-indigo-300">Browse threads</a></p>'
        }
      </div>
    `;
  ...
  api
    ...
    `;
      container
        .querySelector('#retry-bm')
        ?.addEventListener('click', () => renderBookmarks(container));
    });
}
