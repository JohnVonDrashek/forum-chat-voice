...
export function renderThread(container, { threadId }) {
  container.innerHTML =
    '<div class="animate-pulse space-y-3"><div class="h-8 w-64 bg-slate-800 rounded"></div><div class="h-32 bg-slate-800 rounded-xl"></div></div>';
  ...
  Promise.all([
    ...
    `;
      container
        .querySelector('#retry-thread')
        ?.addEventListener('click', () => renderThread(container, { threadId }));
    });
