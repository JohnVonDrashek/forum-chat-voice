...
import { formatDateLabel, formatTime } from '../lib/date.js';
...
export function renderChat(container, { channelId }) {
  ...
  function render() {
    ...
    function sendMessage() {
      ...
      api
        .sendChatMessage({ channel_id: currentChannel.id, author_id: user.id, content })
        .catch(() => {
    ...
    if (currentChannel) {
      ...
      sseCleanup = connectSSE(
        `/api/channels/${currentChannel.slug}/stream`,
        data => {
  ...
  loadChannels()
    ...
    `;
      container
        .querySelector('#retry-chat')
        ?.addEventListener('click', () => renderChat(container, { channelId }));
    });
