...
export function renderSettings(container) {
  ...
  if (!user) {
    container.innerHTML =
      '<p class="text-center py-8 text-slate-400"><a href="/login" class="text-indigo-400">Sign in</a> to access settings.</p>';
    return;
  }
  ...
  function renderNotificationsTab(el) {
    el.innerHTML =
      '<div class="animate-pulse"><div class="h-40 bg-slate-800 rounded-xl"></div></div>';
    ...
    api
        ...
        el.querySelectorAll('.notif-toggle').forEach(btn => {
          btn.addEventListener('click', async () => {
            ...
            btn.querySelector('div').className =
              `w-4 h-4 bg-white rounded-full transform transition-transform ${newEnabled ? 'translate-x-5' : 'translate-x-1'}`;
            try {
      ...
      .catch(() => {
        el.innerHTML = '<p class="text-red-400">Failed to load preferences.</p>';
      });
  }
